...
class ProfileKeychainService {
    ...
    func saveProfile(_ profile: ProfileModel) throws {
        let key = profileKeyPrefix + profile.userIdAndPublickKeyPrefix
        ...
        do {
            ...
            try updateProfileIndex(with: profile.userIdAndPublickKeyPrefix)
        } catch {
            log.error("[Profile] Save profile failed for user \(profile.userIdAndPublickKeyPrefix): \(error)")
