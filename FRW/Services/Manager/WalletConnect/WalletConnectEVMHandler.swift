                ...
                Task {
                  if currentIsCOA(address: fromAddress) {
                    ...
                    if let urlString = try? await web3.provider.url.absoluteString, let rpcURL = URL(string: urlString) {
                         if let fees = try? await fetchGasFee(rpcURL: rpcURL) {
                    ...
                    if let maxFee = maxFeePerGas, let maxPriority = maxPriorityFeePerGas {
                        let maxFeeHex = self.normalizeHexString(String(maxFee, radix: 16))
                        let maxPriorityHex = self.normalizeHexString(String(maxPriority, radix: 16))
                        ...
                        let gasPriceHex = self.normalizeHexString(String(price, radix: 16))
